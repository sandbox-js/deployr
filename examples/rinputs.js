...
/* 
 * @NOTE - First add your DeployR endpoint into the `host`, example:
 *         deployr.configuare( { host: 'http://dhost:dport' } )
 * --------------------------------------------------------------
 *
 * DeployR request to run a repository managed rscript
 * --------------------------------------------------------------
 * 0. configure request to DeployR server `http://dhost:dport`
 * 1. /r/repository/script/execute
 * -------------------------------------------------------------- 
 *
 * .numeric(name, value)
 * .logical(name, value)
...
var RInput = deployr.RInput; // alias
...
deployr.configure( { logging: true, host: config.deployrEndpoint })
  ...
  .factor('input_factor', [1,2,3], false, [4,5,6], ['a', 'b' , 'c'])
  .factor('input_orderedfactor', [1,2,3], true, [4,5,6], ['a', 'b' , 'c'])  
